...
def hu_d03_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create HU_D03 flat terrain velocity configuration."""
  cfg = make_velocity_env_cfg()

  # Sim settings: keep smaller for flat.
  cfg.sim.njmax = 300
  cfg.sim.mujoco.ccd_iterations = 50
  cfg.sim.contact_sensor_maxmatch = 64
  cfg.sim.nconmax = None

  # Disable CCD modes for HU_D03 because foot geoms carry non-zero margins.
  for flag in ("multiccd", "nativeccd"):
    if flag not in cfg.sim.mujoco.disableflags:
      cfg.sim.mujoco.disableflags = (*cfg.sim.mujoco.disableflags, flag)
  ...
  # Remove raycast sensor + height scan observation (no terrain scan needed).
